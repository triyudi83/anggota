                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control select2" name="jabatan" id="jabatan" style="width:100%">
                                    <option value="" data-level="">Pilih Jabatan</option>
                                </select>
                                <span class="text-danger" id="Errorjabatan"></span>
                            </div>
                        </div>
                        <!-- /.box-body -->
                    </div>
            ...
            <div class="box box-success">
                <div class="box-header with-border">
                    <?php if (tomboltambah($activeMenu) == 'aktif') { ?>
                        <a href="<?= site_url('jabatan/create'); ?>" class="btn btn-success ActView"><i class="fa fa-plus" aria-hidden="true"></i> Tambah Data</a>
                    <?php } ?>
                </div>
                <!-- /.box-header -->
                <div class="box-body  table-responsive">
                    <button style="margin-bottom: 10px" class="btn btn-primary delete_all btn-sm" data-url="<?= base_url('jabatan/itemdelete') ?>">Hapus Semua Item</button>
                    <table id="example" class="table table-bordered table-striped" style="width:100%">
                        ...
                        <tbody>
                            ...
                            foreach ($jabatan as $t) {
                                $no++ ?>
                                <tr>
                                    <td><input type="checkbox" class="sub_chk" data-id="<?= $t['id_jabatan'] ?>"></td>
                                    <td><?php echo $no; ?></td>
                                    <td><?php echo $t['nama_susunan']; ?></td>
                                    <td><?php echo $t['level_pengurus']; ?></td>
                                    <td><?php echo $t['jabatan']; ?></td>
                                    <td>
                                        <?php if($t['keterangan'] == '1') {
                                            echo 'Satu Jabatan Satu Orang';
                                        }else{
                                            echo 'Satu Jabatan Banyak Orang';
                                        } ?>
                                        
                                    </td>
                                    <td>
                                        <?php if (tomboledit($activeMenu) == 'aktif') { ?>
                                            <a href="<?= base_url() ?>jabatan/edit/<?= $t['id_jabatan'] ?>" class="btn btn-success btn-sm ActView" title="Edit <?= $activeMenu ?>"><i class="fa fa-pencil" aria-hidden="true"> </i> Edit</a>
                                        <?php } ?>
                                        <?php if (tombolhapus($activeMenu) == 'aktif') { ?>
                                            <a href="<?= base_url() ?>jabatan/remove/<?= $t['id_jabatan'] ?>" class="btn btn-danger btn-sm delete" title="Hapus <?= $activeMenu ?>"><i class="fa fa-trash-o" aria-hidden="true"></i> Hapus</a>
                                        <?php } ?>
                                    </td>
                                </tr>
                            <?php } ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#jabatan').change(function() {
            var id = $(this).val();
            var nama = $(this).find(':selected').data('jabatan');
            tabel.columns(4).search(nama).draw();
        });
    });
</script>